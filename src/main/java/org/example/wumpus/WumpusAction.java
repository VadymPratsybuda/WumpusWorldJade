...
/**
 * https://github.com/aimacode/aima-java/blob/AIMA3e/aima-core/src/main/java/aima/core/environment/wumpusworld/WumpusAction.java
...
public enum WumpusAction {
    ...
    public static WumpusAction fromSymbol(String symbol) {
        for (WumpusAction action : WumpusAction.values()) {
            if (action.getSymbol().equalsIgnoreCase(symbol)) {
                return action;
            }
        }
